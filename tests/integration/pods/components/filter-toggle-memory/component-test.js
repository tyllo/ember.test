...
moduleForComponent('filter-toggle-memory', 'Integration | Component | filter toggle memory', {
  integration: true,
...
test('it renders', function(assert) {
  this.render(hbs`{{filter-toggle-memory model = (readonly model)}}`);
  assert.ok(this.$('ul'));
});
...
test('it should on click checkbox', function(assert) {
  ...
  this.render(hbs`{{filter-toggle-memory
